...
\begin{align*}
    Tx &= \lambda_i x\\
    (T - \lambda_i I)x &= 0\\
    % T - \lambda_i I &=  \begin{bmatrix} 
    %         \lambda_1 - \lambda_i & \cdots \\ 
    %         0 & \lambda_2 - \lambda_i & \cdots \\
    %         \vdots & \vdots & \ddots \\
    %         0 & 0 & & 0 & \cdots \\
    %         \vdots & \vdots & & \vdots & \ddots \\
    %         0 & 0 & & 0 & & \lambda_n - \lambda_i\end{bmatrix}\\
...
In general, the \textbf{QR iteration with shift} process works by shifting the last eigenvalue (smallest in absolute value), updating the shift in each iteration. It works as follows:
\begin{align*}
    1. \;& \mu_k = T_k[n, n]\\
...
\end{align*}
And lastly, another useful speed-up property is called \textbf{deflaction}, which is a process that allows us to break up the current QR iteration process into two smaller/easier problems.
\begin{itemize}
...
\subsubsection{QR iteration on symmetric matrices}
...
% TODO - BOOK CHAPTERS NOT COVERED IN LECTURE
\subsection{Bulge chasing}
\subsection{Fracis shifts}
...
\end{align*}
% TODO -- need to read the textbook
