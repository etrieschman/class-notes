...
Similar to the $LU$ factorization, the \textbf{Gram-Schmidt Transformation} starts with the property that $A=QR$ can be written as a sum of the outer products of the columns of $Q$ and rows of $R$: $A = QR = q_1r_1^T + \dots q_mr_m^T$. The algorithm proceeds as follows
...
\noindent Observe $(A - \mu I)^{-1}$ has the same eigenvectors of $A$:
...
\noindent \textbf{Convergence: } $O((\abs{\frac{\lambda_i - \mu}{\lambda_j - \mu}})^k)$, where $\lambda_j$ is the next closest eigenvalue to $\mu$
